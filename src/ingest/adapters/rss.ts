...
export function createRssAdapter(deps: RssAdapterDeps): IngestAdapter {
  ...
  return {
    ...
    async poll(sources: Source[]): Promise<IngestResult> {
      ...
      // Shuffle sources to mix domains and avoid hammering one host (e.g. Yahoo) all at once
      const shuffled = [...sources].sort(() => Math.random() - 0.5);

      // Concurrency control: 10 workers to avoid triggering 429 Too Many Requests
      ...
      const worker = async () => {
        while (i < shuffled.length) {
          ...
          // Tiny stagger to avoid micro-bursts of requests
